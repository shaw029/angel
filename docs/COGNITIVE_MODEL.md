...
**No content analysis.** Cognitive state is estimated entirely from behavioral signals — scroll velocity, session duration, interaction patterns, structural DOM features. Angel never reads page text, infers topic, or builds a content profile.
...
**Intentional conservatism.** The model errs toward under-classifying problematic states rather than over-firing. A false negative (missing a loop) is preferable to a false positive (interrupting intentional browsing). States require signal consistency across multiple evaluations before transitioning.
...
This state receives `HEALTH_SCORE = 1.0`. Angel never intervenes in this state (`preferredTier: 'none'`). Interventions during intentional browsing would be interruptions, not assistance.
...
`HEALTH_SCORE = 0.8`. Interventions are rare (high confidence threshold, subtle tier only, low session cap). The system mostly observes in this state.
...
`HEALTH_SCORE = 0.2`. This is the primary target state for Angel's interventions. The mechanic is the engagement loop — the content environment is designed to suppress the stop signal.
...
`HEALTH_SCORE = 0.1`. The mechanic is decision pressure — the environment is designed to trigger System 1 responses that bypass System 2 deliberation.
...
`HEALTH_SCORE = 0.35`. The mechanic is attention fragmentation — design choices (notification badges, tab indicators, competing CTAs) that deliberately partition attention across multiple contexts.
...
Transitions use hysteresis to prevent thrashing:
1. The estimated state must differ from the current state
2. The new state must have been consistent for ≥ 2 consecutive evaluations
3. Or a single high-confidence signal (confidence > 0.85) overrides the hysteresis
...
### HEALTH_SCORE Sequence Analysis

For each state in the recent history buffer, its HEALTH_SCORE is extracted. The drift algorithm computes:

- **Delta**: `HEALTH_SCORE[current] - HEALTH_SCORE[oldest_in_window]`
- **Variance**: standard deviation of scores in the window
- **Recency weight**: recent states weighted more heavily than older states

### Trajectory Classification

```
rapid_escalation: delta < -0.4 in last 3 states
escalating:       delta < -0.2 over full window, variance low
recovering:       delta > +0.2, current HEALTH_SCORE > window average
volatile:         variance > 0.25 (frequent oscillation)
stable:           |delta| < 0.1, variance < 0.15
```

### Cooldown Scaling

| Trajectory | Scale | Rationale |
|---|---|---|
| `recovering` | 2.5× | Don't interrupt a correction already in progress |
| `volatile` | 1.8× | User is sensitive; more interventions won't help |
| `stable` | 1.0× | Baseline |
| `escalating` | 0.9× | Slightly more responsive |
| `rapid_escalation` | 0.7× | Lower threshold; earlier, potentially stronger intervention |
